...
def input_data(did):
        command = "select tbli.pk_idx, " \
                  ...
                  " tbli.fd_accelerometer_x as ax, \
                    ...
                    where\
                    tbli.fd_doctor_id = '" + did + "' and tbli.fd_reg_date >= DATE_ADD(NOW(), INTERVAL 0 DAY)\
                    and tbli.fd_reg_date <= DATE_ADD(NOW(), INTERVAL '50' SECOND)\
...
def save_to_server(did,loc):
    ...
    did = "'"+did+"'"
    #
    # print "INSERT INTO tbl_doctor_loc_estimation_info\
    # (fd_date, fd_doctor_id, fd_loc_cd, fd_reg_date) VALUES (%s, %s, %s, NOW());" %(str_date, did, loc)
    cur.execute("INSERT INTO tbl_doctor_loc_estimation_info (fd_date, fd_doctor_id, fd_loc_cd, fd_reg_date) VALUES (%s, %s, %s, NOW());" %(str_date, did, loc))
    # cur.execute("INSERT INTO test_table (fd_date, fd_doctor_id, fd_loc_cd, fd_reg_date) VALUES (%s, %s, %s, NOW());" % (str_date, did, loc))
    db.commit()
...
def extract_data_oned(did, states=None, labels=None, groups=None, personIDs=None, numRows=None, numData=None, mode=None, NUM_CHANNELS=None,
                     ONED=True, DATA_SIZE=None):

    results = input_data(did)
    x_data = numpy.array([])
