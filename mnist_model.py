...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope("fully_connected1"):
                FC_W1 = tf.get_variable("FC_W1", shape=[784, 300],
                                     initializer=tf.contrib.layers.xavier_initializer())
                FC_b1 = tf.get_variable("FC_b1", shape=[300])
                FC_L1 = tf.matmul(self.X, FC_W1) + FC_b1
            ...
            with tf.name_scope("fully_connected3"):
                ...
                FC_W3 = tf.get_variable("FC_W3", shape=[300, 10],
                                     initializer=tf.contrib.layers.xavier_initializer())
    ...
    def predict(self, x_test, keep_prop=1.0):
        return self.sess.run(self.logits, feed_dict={self.X: x_test, self.keep_prob: keep_prop, self.phase:False})

    def get_accuracy(self, x_test, y_test, keep_prop=1.0):
        return self.sess.run([self.accuracy, self.X, self.Y, self.logits], feed_dict={
            self.X: x_test, self.Y: y_test, self.keep_prob: keep_prop, self.phase:True})

    def train(self, x_data, y_data, keep_prop=0.7):
        return self.sess.run([self.merged_summary, self.cost, self.optimizer, self.accuracy], feed_dict={
            self.X: x_data, self.Y: y_data, self.keep_prob: keep_prop, self.phase:True})