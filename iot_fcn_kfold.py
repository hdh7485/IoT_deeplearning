...
def main():
    ...
    beacon_split_table = data.expand_time_onehot_beacon_table(beacon_table, 3)[:, :, 1:, np.newaxis]
    target_split_table = data.expand_time_onehot_beacon_table(target_table, 3)[:, -1, 1:]

    trainX, testX, trainY, testY = train_test_split(
        beacon_split_table, target_split_table, test_size=0.15, shuffle=False)
    print("trainX:", np.shape(trainX), "testX:", np.shape(testX))
    print("trainY:", np.shape(trainY), "testY:", np.shape(testY))
    # initialize

    sess = tf.Session()
    m1 = Model(sess, "m1", 0.001)
